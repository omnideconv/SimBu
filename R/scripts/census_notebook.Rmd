...
## Census for Maynard dataset (TPMs calculated with salmon)
```{r}
...
Compare Maynard census counts with number of reads in cells
```{r}
...
maynard_both <- merge(maynard_census, maynard_reads, by.x="cell", by.y="Run")

png(filename="../plots/maynard_census_vs_reads.png", width=1800, height=1200)
ggplot(maynard_both, aes(x=census, y=n_reads))+
  geom_point(aes(color=cell_type), alpha=0.75)+
