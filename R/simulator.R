...
calc_scaling_vector <- function(data, scaling_factor, custom_scaling_vector, scaling_factor_single_cell, BPPARAM, run_parallel){
  ...
  if(scaling_factor == "census"){
    ...
    if(!scaling_factor %in% colnames(SummarizedExperiment::colData(data))){em <- paste0("A column with the name ", scaling_factor," cannot be found in the annotation."); stop(em)}
    if(!is.numeric(SummarizedExperiment::colData(data)[,c(scaling_factor)])){em <- paste0("The column with the name ", scaling_factor," is not numeric and cannot be used for scaling."); stop(em)}
...
merge_scaling_factor <- function(data, scaling_factor_values, scaling_factor_name){
  ...
  if(length(type_intersection) == 0){em<- paste0('Could not apply ',scaling_factor_name,' scaling factor, because none of its cell types are present in the provided dataset.'); stop(em)}
  ...
  if(length(missing_types) != 0){wm <- paste0('For some cell type(s) in the dataset, no scaling factor is available when using ',scaling_factor_name, ': ',missing_types,'. This cell type will not be re-scaled.'); warning(wm)}
