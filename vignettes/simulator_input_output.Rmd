...
Note: if you want both counts and tpm data as input, you will have to provide two files; the annotation layer `obs` has to match between these files, otherwise the intersection will be used. You can provide the column names in `obs` which store cell id and cell type with the `cell_id_col` and `cell_type_col` parameters respectively. 
...
ds_h5ad <- SimBu::dataset_h5ad(h5ad_file_counts = counts_file,
                               name = "h5ad_dataset",
                               cell_id_col = 0,                  # this will use the row names of the metadata as cell identifiers
                               cell_type_col = 'leiden')         # this will use the 'leiden' column of the metadata as cell type info
