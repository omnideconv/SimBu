...
The pseudo-bulk sample will be generated by aggregating the expression values from all sampled cells and is then normalized. The default aggregation method is `mean`, but you also have the options `sum` and `median`. You can select them like this:
