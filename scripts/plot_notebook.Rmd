...
```{r}
suppressWarnings(source("load_scaling_factors.R"))
...
# vento_tormo_decidua_reads_f <- merge(vento_decidua_reads, wanted_types, by="type", all.y = T)
# vento_tormo_decidua_reads_f$source <- rep("vento_tormo (reads)", length(wanted_types$type))
# vento_tormo_decidua_reads_f[vento_tormo_decidua_reads_f$type %in% c("T cells CD4", "T cells CD8", "T regulatory cells")]$mRNA <- vento_tormo_decidua_reads_f[vento_tormo_decidua_reads_f$type == "T cells"]$mRNA
# vento_tormo_decidua_census_f <- merge(vento_decidua_census, wanted_types, by="type", all.y = T)
# vento_tormo_decidua_census_f$source <- rep("vento_tormo (census)", length(wanted_types$type))
# vento_tormo_decidua_census_f[vento_tormo_decidua_census_f$type %in% c("T cells CD4", "T cells CD8", "T regulatory cells")]$mRNA <- vento_tormo_decidua_census_f[vento_tormo_decidua_census_f$type == "T cells"]$mRNA
...
data_lst <- list(maynard_reads_f, maynard_census_f, quantiseq_f, epic_f, miltenyi_f, vento_tormo_reads_f, vento_tormo_census_f, vento_tormo_psmb2_f, monaco_f,hao_reads_f,hao_census_f,hao_psmb2_f)
...
corr <- final_corr$r-0.000001

png(filename="/home/simulator/plots/census_correlation.png", width = 800, height = 600)
corrplot(corr,
         tl.col = "black",
